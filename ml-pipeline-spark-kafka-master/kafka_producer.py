...
import json
import requests
from kafka import KafkaProducer #classe KafkaProducer permet d'envoyer des messages à un cluster Kafka
...
API_URL = 'http://streaming-api:8000/Streaming'
MAX_RETRIES = 5
RETRY_DELAY = 10

class StreamProducer: # classe responsable de connexion à Kafka, de la récupération de données depuis l'API et de l'envoi de ces données à Kafka
    def __init__(self, bootstrap_servers: str, topic: str): # constructeur initialise producteur Kafka avec serveur de Kafka et nom du topic
        self.topic = topic
    ...
    def stream_data(self, url: str) -> Iterator[Dict[str, Any]]:
        ...
        while True:
            try:
                with requests.get(url, stream=True) as response: #utilise requests pour se connecter à l'API et récupérer les données en streaming. Les données sont renvoyées sous forme de dictionnaire Python après les avoir décodées depuis JSON
    ...
    def send_to_kafka(self, data: list, partition: int = 0): # méthode envoie les données récupérées dans le topic Kafka spécifié
